...
| Akash | DePIN | Decentralized cloud (SDL-based) |
...
| Inferia Diffusion | Image generation |

### External Provider Proxying
Route requests to external providers through InferiaLLM for unified access control and auditing:
- OpenAI, Anthropic, Gemini, Groq, Cerebras, Mistral, DeepSeek
...
### Document Management
- Upload PDF, DOCX, TXT, MD, CSV, JSON files (up to 50 MB)
...
- Actor identification with user email resolution
- Organization-scoped logs for multi-tenant isolation
- Category filtering via Dashboard UI
- Immutable history
