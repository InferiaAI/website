...
The system supports ingesting text from various file formats (up to 50 MB per file):
- Plain Text (`.txt`)
- Markdown (`.md`)
- CSV (`.csv`)
- JSON (`.json`)
...
```bash
curl -X POST "http://localhost:8000/knowledge/data/upload" \
...
```bash
curl -X GET "http://localhost:8000/knowledge/data/collections" \
  -H "Authorization: Bearer $JWT_TOKEN"
```
...
```bash
curl -X GET "http://localhost:8000/knowledge/data/collections/product-manuals/files" \
  -H "Authorization: Bearer $JWT_TOKEN"
```
