...
Returns available models/deployments for the authenticated API key.
...
Generate text embeddings using configured embedding models.

### Image Generation

| Method | Endpoint | Description |
| --- | --- | --- |
| POST | `/v1/images/generations` | Generate images (DALL-E style) |
...
| POST | `/v1/videos/edits` | Edit existing videos |
| POST | `/v1/videos/extensions` | Extend existing videos |

## Client IP Forwarding

If requests reach the Inference Gateway through proxies or load balancers, forward client IP with one of these headers (checked in order):
...
The value is stored in `inference_logs.ip_address` and used for IP-based filtering in Insights.

## Request Lifecycle

1. **Authentication**: Validate API key
2. **Context Resolution**: Resolve deployment config from API Gateway
