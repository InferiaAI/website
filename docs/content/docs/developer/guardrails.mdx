...
description: "Configuring safety checks and guardrail policies"
---

The Guardrail Engine (port 8002) enforces safety policies through a pipeline of guardrails. These checks happen before the prompt reaches the LLM (Input Guardrails) and after the LLM generates a response (Output Guardrails).
...
The Guardrail Engine utilizes **Parallel Execution** to minimize latency.
...
Create a new file in `package/src/inferia/services/guardrail/providers/` (e.g., `regex_provider.py`).
...
Update `package/src/inferia/services/guardrail/engine.py` to initialize your new provider.
